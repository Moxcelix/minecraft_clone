...
float PerlinNoise::octave_noise(float x, float y, float z, int octaves) 
{
	float eps = 1;
	int mult = 1;
	float result = 0;
	...
	{
		eps *= 0.5;
		result += noise(x * mult, y * mult, z * mult) * eps;
		mult *= 2;
	}
...
float PerlinNoise::octave_noise(float x, float y, int octaves)
{
	float eps = 1;
	int mult = 1;
	float result = 0;
	...
	{
		eps *= 0.5;
		result += noise(x * mult, y * mult, 0) * eps;
		mult *= 2;
	}
